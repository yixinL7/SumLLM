...
def generate_summaries_test(args):
    ...
    base_dir = "/".join(args.model_dir.split("/")[:-1])
    model.eval()
    ...
    # quit()
    with open(args.src_dir) as source, open(os.path.join(base_dir, args.tgt_dir), 'w') as fout:
        ...
        if slines != []:
            ...
            for hypothesis in dec:
                ...
                fout.write(hypothesis + '\n')
                fout.flush()
        with open(args.ref_dir) as target, open(os.path.join(base_dir, args.tgt_dir)) as pred:
            cnt = 0
...
if __name__ ==  "__main__":
    ...
    parser.add_argument("--end", type=int, default=1000, help="end")
